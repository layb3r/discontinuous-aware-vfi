...
def train(ppl, dataset_cfg_dict, optimizer_cfg_dict):
    ...
    dataset_train = AugVimeo(dataset_name='train', data_root=data_root)
    dataset_val = AugVimeo(dataset_name='validation', data_root=data_root)
    ...
    while step <  total_step+1:
        ...
        for data in train_data:
            ...
            img1 = data_gpu[:, 3:6]
            gt = data_gpu[:, 6:9]
            m0 = data_gpu[:, 9:10] # dimension: [B, 1, H, W]
            m1 = data_gpu[:, 10:11] # dimension: [B, 1, H, W]
...
def evaluate(ppl, step, val_data, writer_val, is_write=True):
    ...
    for i, data in enumerate(val_data):
        ...
        with torch.no_grad():
            pred, extra_dict = ppl.inference(img0, img1, m0, m1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_file', type=str, default="",
            help='pretrainded weight path, used when load_pretrain is True')
