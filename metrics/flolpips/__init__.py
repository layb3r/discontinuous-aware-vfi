...
class LPIPS(nn.Module):
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        for kk in range(self.L):
            feats0[kk], feats1[kk] = utils.normalize_tensor(
                outs0[kk]
            ), utils.normalize_tensor(outs1[kk])
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            ...
            value = utils.l2(
                utils.tensor2np(utils.tensor2tensorlab(in0.data, to_norm=False)),
...
class DSSIM(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            value = utils.dssim(
                1.0 * utils.tensor2im(in0.data),
                1.0 * utils.tensor2im(in1.data),
                range=255.0,
            ).astype("float")
        elif self.colorspace == "Lab":
            value = utils.dssim(
...
class FloLPIPS(LPIPS):
    ...
    def forward(self, in0, in1, flow, retPerLayer=False, normalize=False):
        ...
        res = [
            mw_spatial_average(self.lins[kk](diffs[kk]), flow, keepdim=True)
            for kk in range(self.L)
        ]
...
def calc_flolpips(disMP4, refMP4, tempDir, hwt):
    batch_size = 8

    # convert to yuv first
    # os.system(
    #     f"ffmpeg -hide_banner -y -loglevel error -i {refMP4} {tempDir}/flolpips_ref.yuv"
    # )
    # os.system(
    #     f"ffmpeg -hide_banner -y -loglevel error -i {disMP4} {tempDir}/flolpips_dis.yuv"
    # )
    ...
    # cap_dis = cv2.VideoCapture(disMP4)
    # cap_ref = cv2.VideoCapture(refMP4)
    # assert int(cap_dis.get(cv2.CAP_PROP_FRAME_COUNT)) == int(
    #     cap_ref.get(cv2.CAP_PROP_FRAME_COUNT)
    # )
    ...
    height, width, num_frames = hwt
    stream_dis = open(f"{disMP4}", "r")
    stream_ref = open(f"{refMP4}", "r")

    flolpips_list = []
    batch_ref_list, batch_dis_list = [], []
    batch_ref_next_list, batch_dis_next_list = [], []
    for iFrame in range(num_frames - 1):
        frame_dis = TF.to_tensor(
            utils.read_frame_yuv2rgb(stream_dis, width, height, iFrame, 8, "420")
        )
        frame_dis_next = TF.to_tensor(
            utils.read_frame_yuv2rgb(stream_dis, width, height, iFrame + 1, 8, "420")
        )
        frame_ref = TF.to_tensor(
            utils.read_frame_yuv2rgb(stream_ref, width, height, iFrame, 8, "420")
        )
        frame_ref_next = TF.to_tensor(
            utils.read_frame_yuv2rgb(stream_ref, width, height, iFrame + 1, 8, "420")
        )
        ...
        if len(batch_ref_list) % batch_size == 0:
            with torch.no_grad():
                ...
                flolpips = loss_fn.forward(
                    frames_ref, frames_dis, flow_diff, normalize=True
                )
